    ...
    <div className="space-y-2">
      {categories.map(([cat, total]) => {
        ...
        const monthTransactions = transactions.filter(
          (t) => t.type === 'expense' && t.category === cat && t.date.startsWith(currentMonth)
        )
          ...
          <div key={cat}>
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-gray-500">({monthTransactions.length})</span>
              </div>
              <div className="flex items-center gap-2">
            ...
            {isExpanded && monthTransactions.length === 0 && (
              <div className="ml-4 mt-2 p-2 text-sm text-gray-400 bg-gray-50 rounded">
                Aucune dépense ce mois-ci dans cette catégorie
              </div>
            )}
          </div>
        )
